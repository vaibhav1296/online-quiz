...
validateAdminAPI.validateAddTopicRequest = (req, res, next)=>{
    ...
    if(validateChecksObject.isNullOrUndefined(topicName) || !validateChecksObject.isString(topicName)){
...
validateAdminAPI.validateAddQuestionRequest = (req, res, next)=>{
    ...
    }else if(validateChecksObject.isNullOrUndefined(question.title) || !validateChecksObject.isString(question.title)){
        throw onlineQuizError("The title is empty/invalid",400)
    }else if(validateChecksObject.isNullOrUndefined(question.answer) || !validateChecksObject.isString(question.answer)){
